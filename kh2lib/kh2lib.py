...
class kh2lib:
    ...
    def lookupObject(self, mdlxname):
        for obj in self.objects:
            if mdlxname == obj["mdlx"]:
                return obj["name"]
    ...
    def make_mod(self, moddir='autogen_mod', title="Autogenerated mod", description="Autogenerated by kh2lib", originalAuthor="kh2lib"):
        import shutil

        files = self.get_git_modifications()
        ...
        for fn in files:
            newfn = fn.replace("/", "\\")
            destdir = os.path.join(moddir, os.path.dirname(fn))
            if not os.path.exists(destdir):
                os.makedirs(destdir)
            sourcename = os.path.join(self.gamedir, fn)
            destname = os.path.join(destdir, os.path.basename(newfn))
            ...
            mod_yaml["assets"].append(f_yaml)
        import yaml
        yaml.dump(mod_yaml, open(os.path.join(moddir,"mod.yml"), "w"))
        mod_yaml
